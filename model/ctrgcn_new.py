...
class DPP_GCN(nn.Module):
    ...
    def __init__(self, num_class=2, num_point=17, num_person=1, graph=None, graph_args=dict(), in_channels=2,
        ...
        # 3a. 对抗性多任务头 (L_adv)
        # 这是一个多头投影层，用于将特征投影到与文本原型相同的维度 (proto_dim)
        # num_confounders = 4 (例如: 角度, 服装, 高度, 方向)
        self.adv_head = nn.ModuleList([
            nn.Linear(encoder_out_dim, proto_dim) for _ in range(num_confounders)
        ])

        # === 新增：对抗性分支的 logit_scale ===
        # 我们为每个混淆因素(num_confounders)创建一个可学习的 logit_scale
        # 使用 nn.ParameterList 来确保它们能被优化器正确注册
        self.logit_scale_adv = nn.ParameterList([
            nn.Parameter(torch.ones([]) * np.log(1 / 0.07)) for _ in range(num_confounders)
        ])
    ...
    def forward(self, x, grl_alpha=1.0):
        ...
        # 3b. 对抗性特征 (用于 L_adv)
        adv_features = []
        for adv_projector in self.adv_head:
            adv_features.append(adv_projector(f_global_adv))
        ...
        return logits_ce, feature_dict, self.logit_scale, part_features, adv_features, self.logit_scale_adv